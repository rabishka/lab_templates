...
template <typename T> Optional<T>::operator bool() const { return has_value_; }

template <typename T>
T& Optional<T>::value() {
    if (!has_value_) throw std::runtime_error("error");
...
template <typename T>
T Optional<T>::value_or(const T& d) const {
    return has_value_ ? *reinterpret_cast<const T*>(storage_) : d;
}

template <typename T> T& Optional<T>::operator*() { return *reinterpret_cast<T*>(storage_); }
template <typename T> const T& Optional<T>::operator*() const { return *reinterpret_cast<const T*>(storage_); }
template <typename T> T* Optional<T>::operator->() { return reinterpret_cast<T*>(storage_); }
template <typename T> const T* Optional<T>::operator->() const { return reinterpret_cast<const T*>(storage_); }
...
template <typename T>
bool Optional<T>::operator!=(const Optional& rhs) const { return !(*this == rhs); }
